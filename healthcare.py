# demo_single_method_sklearn_examples_separate.py
# Single AI method: one MLPRegressor that predicts intent (one-hot) + satisfaction score
# Modified to display the two AI tasks separately for each example:
#   - Input
#   - Classification: Output label and per-example metric (correct / incorrect as %)
#   - Regression: Output score and per-example metric (absolute error and % of 0-10 range)
# Also prints overall metrics (classification accuracy %, regression R^2 % and RMSE)
# Usage: python demo_single_method_sklearn_examples_separate.py
...
DATA_CSV = "synthetic_dataset_regress.csv"  # generated by generate_dataset_regress.py
